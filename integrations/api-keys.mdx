...
Two things that commonly trip people up on the first run:

- `lead_source` belongs to a **company**, not a person. Sending it to `create_person` fails with `Lead source belongs to a Company`.
- `create_deal` needs the person to exist first, because `party.id` is the id from the `create_person` response. Create the person, read the id out of the result, then create the deal.
